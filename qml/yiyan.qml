...
Widget {
    ...
    // 内容是否超出可视高度（超出才滚动）
    property bool overflow: backend.yiyanStatus === "ok" && col.height / 2 > scrollBox.height
    ...
    Quick.Text {
        ...
        visible: backend.yiyanStatus !== "ok"
        text: backend.yiyanStatus === "error" ? qsTr("网络连接异常，5分钟后自动重试") : qsTr("加载中，请稍后...")
    ...
    Item {
        ...
        Connections {
            target: backend
            function onYiyanStatusChanged() { scrollBox.restartScroll() }
            function onYiyanContentChanged() { scrollBox.restartScroll() }
        }
